// ============================================
// ARQUIVO: js/app.js (CONSOLIDADO B) — FIX FINAL
// - Premium: NUNCA mostra badge azul / créditos (nem flash no refresh)
// - Não Premium: mantém créditos + botão Ativar Premium
// - Carrega credits + unlocked do storage (antes não carregava)
// ============================================

/* ===============================
   BOOTSTRAP PREMIUM (B) + ANTI-FLASH
   =============================== */
let isPremium = false;
...
// ANTI-FLASH: trava header até o JS renderizar o estado final
(function bootHeaderState() {
  try {
    const root = document.documentElement;
    // garante que o header não "pisque" o estado errado
    root.classList.add('booting');
    // se já sabemos que é premium, já marca antes de renderizar
    if (isPremium) root.classList.add('premium-preload');
  } catch (e) {}
})();
...
async function loadUserData() {
  try {
    ...
    if (creditsResult && creditsResult.value) {
      const parsed = parseInt(creditsResult.value, 10);
      if (!Number.isNaN(parsed)) credits = parsed;
    }

    // unlocked recipes
    const unlockedResult = await storage.get('fit_unlocked');
    if (unlockedResult && unlockedResult.value) {
      try { unlockedRecipes = JSON.parse(unlockedResult.value) || []; } catch (e) {}
    }
  ...
  // libera a UI só depois de renderizar o estado FINAL (anti-flash)
  try {
    const root = document.documentElement;
    root.classList.remove('booting');
    root.classList.remove('app-loading');
    root.classList.remove('premium-preload');
    root.classList.remove('boot-premium');
  } catch (e) {}
}
...
async function activatePremium() {
  ...
  try {
    ...
    // atualiza UI e garante que nunca volte a mostrar azul
    try {
      document.documentElement.classList.add('premium-preload');
    } catch (e) {}
